...
class Init
{
    ...
    {
        if (\file_exists(str_replace('\\', '/', \plugin_dir_path(dirname(__FILE__))).'assets/js/app.js')) {
			\wp_enqueue_script(__NAMESPACE__.'-script', \plugin_dir_url(dirname(__FILE__)).'assets/js/app.js');
        }
        if (\file_exists(str_replace('\\', '/', \plugin_dir_path(dirname(__FILE__))).'assets/css/app.css')) {
